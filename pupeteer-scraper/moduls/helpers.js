...
function appendObjToJsonFile(obj, jsonFile){
    let hotelInfosJSON = fs.readFileSync(jsonFile, 'utf-8');
    let hotelInfosObj = JSON.parse(hotelInfosJSON);
    let newHotelInfosObj = Object.assign(hotelInfosObj, obj);
    let newHotelInfosJSON = JSON.stringify(newHotelInfosObj);
    
    fs.writeFileSync(jsonFile, newHotelInfosJSON, (err) => {  
        if (err) throw err;
        console.log('new obj appended saved!');
    });
}

function writeObjIntoJsonFile(obj, pathAndName){
    let allHotelsInfosJSON = JSON.stringify(obj);
    fs.writeFile(pathAndName, allHotelsInfosJSON, (err) => {  
        if (err) throw err;
...
async function getNextPageBtnObj(page, currPagiBtnInnerText, pagiBtnsObj) {
    console.log('currPagiBtnInnerText:');
    console.log(currPagiBtnInnerText);
    var nextPagiBtnObj = await page.evaluate(function(currPagiBtnInnerText, pagiBtnsObj){
        ...
        console.log('allPagiBtns:');
        console.log(allPagiBtns);
        console.log('allPagiBtns:');
        console.log(allPagiBtns.length);
...
async function extractHotelCards(page, hotelsCardsObj){
    ...
    for (let [i, hotelCard] of allHotelCardOnPage.entries()) {
        ...
        try {
            ...
            //Get rid of trailing linebrakes
            hotelName = hotelName.replace(/^\s+|\s+$/g, '').toLowerCase();
...
module.exports.writeObjIntoJsonFile = writeObjIntoJsonFile;
module.exports.appendObjToJsonFile = appendObjToJsonFile;
